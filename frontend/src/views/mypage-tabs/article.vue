<template>
  <div class="container p-5">
    <div style="margin-left: 3.5rem; padding-top: 5rem;">
      ...
      <p class="medium d-inline text-white" style="font-size: 1.5rem;">
        포스트
      </p>
      <p class="light text-white ml-4" style="font-size: 1.1rem;">
        내가 작성한 포스트를 모아서 볼 수 있어요
      </p>
    </div>
    ...
    <v-row class="justify-content-center">
      <v-col cols="12" md="11">
        ...
        <div class="blog-card" v-for="post in myPostList" :key="post.pid">
          ...
          <!-- 텍스트 섹션 -->
          <div
            @click="showMyDetail(post.pid)"
            class="description"
            style="cursor: pointer;"
          >
            <p class="medium mb-2" style="font-size: 1.5rem;">
              {{ post.title }}
            </p>
            <div class="under-line"></div>
            <p class="regular" style="font-size: 0.9rem;">
              {{ post.description }}
            </p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    showMyDetail(pid) {
      ...
      this.$router.push({ name: "readPost", params: { pid: pid } });
    },
    async getTags(pid) {
