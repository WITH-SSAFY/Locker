...
<script>
  ...
  export default {
    data: () => ({
      ...
      emailRules: [
        v => !!v || '이메일을 입력해주세요',
        v => /^[A-Za-z0-9_.-]+@[A-Za-z0-9-]+\.[A-Za-z0-9]+/.test(v) || '이메일 형식에 맞게 입력해주세요',
      ],
      usrName : '',
      usrNameRules : [
        v => !!v || '이름을 입력해주세요',
      ...
      nickNameRules : [
        v => !!v || '닉네임을 입력해주세요',
        v => (v &&v.length>=1&& v.length <= 30) || '닉네임은 최소 1자 최대 30자 입니다',
      ...
      passwordRules: [
        v => !!v || '비밀번호를 입력해주세요',
        v => (v &&v.length>=4&& v.length <= 50) || '비밀번호는 최소 4자 최대 50자 입니다',
      ],
      passwordConfirm: '',
    ...
    computed: {
        ...
        passwordConfirmRules(){
            ...
            return[
                () => (this.password===this.passwordConfirm) || "비밀번호가 일치하지 않습니다",
                v => !!v || "비밀번호 확인을 입력해주세요",
                v => (v &&v.length>=4&& v.length <= 50) || '비밀번호 확인은 최소 4자 최대 50자 입니다',
              ...
              if(this.email!=''&&/^[A-Za-z0-9_.-]+@[A-Za-z0-9-]+\.[A-Za-z0-9]+/.test(this.email)){
                  ...
                  axios
                    ...
                    .then(response => {
                        ...
                        console.dir(data);
                        if(data.success){//중복된 닉네임이 없다
                            this.useThisEmail = confirm("해당 이메일을 사용하시겠습니까?");
                        }
                    })
                    .catch(exp =>{ alert("중복된 이메일 입니다."); exp;});
                }
          },
      requestJoin(){//회원 가입 요청
